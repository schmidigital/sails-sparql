...
#      console.log(res.statusCode)
  ...
  insert : ( query, cb ) ->
    ...
    console?.log query if @log_query is true # quick n' dirty logging option
    username = 'mindbabble';
    password = '5YeW4mdLCIi8qfEYG4C4';
    auth = 'Basic ' + new Buffer(username + ':' + password).toString('base64');
#    console.log querystring.stringify (query:query)
    opts =
      uri: "http://mindbabble.com:8890/DAV/home/mindbabble/yy"
      headers:
#     'content-type':'application/x-www-form-urlencoded'
...
#      body: querystring.stringify (query:query)
      body: query,
#      body: "prefix mb: <http://rdf.mindbabble.com/ns/> INSERT DATA { GRAPH <mindbabble> { mb:node a owl:Class . mb:node.label a owl:DatatypeProperty; rdfs:domain mb:node ; rdfs:range xsd:string . mb:node.createdAt a owl:DatatypeProperty; rdfs:domain mb:node ; rdfs:range xsd:dateTime . mb:node.updatedAt a owl:DatatypeProperty; rdfs:domain mb:node ; rdfs:range xsd:dateTime . } }"
      encoding: 'utf8',
      Authorization: auth
    request.post opts, (err, res, body) ->
      if res?
