...
module.exports = (function() {
	...
	var adapter = {
		...
		registerConnection: function(connection, collection_list, cb) {
			...
			var arguments = [ __dirname + '/create_db.js',
												connection.database,
												connection.endpoint,
												connection.username,
												connection.password]

			var response = exec('node', arguments).toString()
			
			console.log(response)

			if (response.indexOf("409") > -1) {
				console.log("Datenbank existiert bereits")
			}
			else if (response.indexOf("201") > -1) {
				console.log("Datenbank erfolgreich angelegt")
			}
			else {
				console.log("Irgendwas war beim Erstellen der Datenbank nicht in Ordnung")
			}
