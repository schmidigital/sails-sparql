...
module.exports = (function() {
	...
	var adapter = {
		...
		registerConnection: function(connection, collection_list, cb) {
			...
			shared.connections[connection.identity].connection.setEndpoint(connection.endpoint);
			shared.connections[connection.identity].connection.setCredentials(connection.username, connection.password);
			shared.connections[connection.identity].connection.setReasoning(connection.reasoning);

			// Create DB if not exists
			//TODO Funktioniert noch nicht. POST Request endet in Timeout. GET geht jedoch!
			//shared.connections[connection.identity].connection.listDBs( function (dblist) {
      //
			//	// check if graph exists and create new one.
			//	if (dblist.databases.indexOf( connection.database ) > -1) {
			//		console.log("Graph exists already! Not creating...")
			//		cb();
			//	} else {
			//		console.log("Graph not exists! Creating DB...")
			...
			//shared.connections[connection.identity].connection.createDB( options, function (data, response) {
			//	console.log(data)
