...
module.exports = (function () {
	...
	var defaults = {
		...
		endpoint: 'http://mindbabble.com:8890/sparql',
...
//        shared.connections[connection.identity].connection.setEndpoint(
...
//        triples += "\n}\n}";
			...
			// Start Query
			helper._insert(conn, triples, function (err, res) {
				conn.database = original_database;
				cb(null, true)
			})
		},
		...
		drop: function (connection, collection, relations, cb) {
			...
			//TODO Drop specific Classes / Graph instead of everything :-)
...
//        console.log(criteria.joins[0].select)
//      },
...
//      console.log(values)
      ...
      // Check if we have abstract class and generate human-friendly id for the class
      var id = "m." + shortid.generate();
      if (!values.abstract)
        id = "m." + shortid.generate();
      else {
				id = collection + "." + values.label.toLowerCase();
...
//        var triples = "@prefix " + conn.prefix.abr + " <" + conn.prefix.val + "> . \n";
			...
			var triples = helper.model_to_turtle(connection, collection, values, options);
...
//			var coll = shared.collections[collection];
			...
			var query = "";
			query += helper._select(connection, collection);
          ...
          helper._query(conn, query, function (err, result) {
						...
						obj = helper.json_to_model(connection, collection, result);
						...
						for (value in values) {
							old_values[value] = obj[0][value];
						}

						var triples_to_delete = helper.model_to_turtle(connection, collection, old_values, options);
				...
				insert_triples: function (callback) {
					...
					var triples_to_insert = helper.model_to_turtle(connection, collection, values, options);
        ...
        helper._query(conn, query, function (err, result) {
					res = helper.json_to_model(connection, collection, result)
					cb(null, res)
				})
			})
		...
		destroy: function (connection, collection, options, values, cb) {
			...
			this.find(connection, collection, options, function (err, res) {
				...
				// Then destroy
				var triples = helper.model_to_turtle(connection, collection, res, {delete: true});
