		...
		function deep(branch, parent_condition, deep_collection, stack, offset) {
			...
			for (var condition in branch) {
				...
				// TODO: Anfragen nach Variablen richtig umwandeln, z.B. ?a mb:user.name "Egon" -> ?a mb:user.name "Egon"^^xsd:string
					...
					var type = coll._attributes[condition].type;
					console.log(type);
					...
					// Wert ist ein DatatypeProperty (Konkreter Wert, z.B. String) oder ObjectProperty (Beziehung zu anderem Knoten)
					else if (type in types.toSparql) {
						...
						if (condition != "id") {
							...
							var attributes = shared.collections["userTable"]._attributes;
							...
							// Datatype Property
							if (shared.properties[deep_collection + "." + condition] >= 0) {
							//if (shared.properties[deep_collection + "." + condition] && _.indexOf(literalClasses, shared.properties[deep_collection + "." + condition].range) >= 0) {
				...
				if (attributes[property] || (columnName = _.findKey(attributes, {
					...
					if (!(attributes[property_key].model || attributes[property_key].collection)) {
						...
						*/
						...
						var type = attributes[property].type;
						log(attributes[property]);
						log(value);



						triples += property_string + ' ' + types.toSparql[type](value) + ' ' + end + '\n';
