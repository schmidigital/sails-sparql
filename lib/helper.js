        ...
        for (result in results) {
            ...
            for (attribute in res) {
				...
				if (attributes[attribute.replace(coll.adapter.identity + "_", '')]) {
					attribute_name = attribute.replace(coll.adapter.identity + "_", '');
				} else {
					attribute_name = attribute.replace(coll.adapter.identity + "_", '');
				...
				if (is_valid) {
					...
					// TODO if (options.select) { checken, damit nur die nötigen Werte geparst werden
					// Attribut ist nicht die ID
					if (attribute_name != "id") {
						...
						if (attributes[attribute.replace(coll.adapter.identity + "_", '')]) {
							obj[attribute_name] = value === undefined ? null : types.fromSparql[type](value)
						} else {
							obj[attribute_name] = types.fromSparql["float"](value);
						}
