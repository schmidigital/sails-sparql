...
//}
  ...
  _query: function (conn, query, options, cb) {
    ...
    query = "DEFINE input:inference <" + conn.database + ".schema> \n" +
...
//  });
   ...
   _insert: function (conn, triples, options, cb) {
    ...
    if (typeof options == 'function') {
      ...
      options = (debug.define == true) ? {debug: true} : {debug: false};
    }

    // Create the Query
    var query = "prefix " + conn.prefix.abr + " <" + conn.prefix.val + ">  \n";
    query += "INSERT DATA { GRAPH <" + conn.database + "> {\n";
    query += triples;
    query += "\n}\n}";

    if (debug.insert == true && options.debug == true) {
      console.log("\n######### Insert Start #########".green);
      console.log(query);
...
//		});
  ...
  _delete: function (conn, triples, cb) {

//  console.log(triples)

    // Create the Query
    var query = "prefix " + conn.prefix.abr + " <" + conn.prefix.val + ">  \n";
    query += "DELETE DATA { GRAPH <" + conn.database + "> {\n";
    query += triples;
    query += "\n}\n}";
...
//						console.log(deep_collection);
         ...
         */
           ...
           */
          var condition_property = (coll._attributes[condition] && coll._attributes[condition].as
            ? coll._attributes[condition].as
            : deep_collection + "." + condition)
...
//              console.log(shared.properties[deep_collection + "." + condition])
    ...
    for (attribute in coll._attributes) {
      if (attribute != "id") {
        ...
        var attribute_property = (coll._attributes[attribute].as
          ? coll._attributes[attribute].as
          : conn.prefix.abr + collection + "." + attribute)
        ...
        if (!coll._attributes[attribute].collection) {
          ...
          if (shared.properties[conn.prefix.abr + collection + "." + attribute]
            && _.indexOf(literalClasses, shared.properties[conn.prefix.abr + collection + "." + attribute].range) >= 0
            && shared.properties[conn.prefix.abr + collection + "." + attribute].multilang) {
...
//};
  ...
  /*
   *   Converts the JSON results of stardog to waterline model
   */
...
//  results = raw_data.results.bindings; // Stardog
   ...
   */
  model_to_turtle: function (connection, collection, models, options) {
...
//    delete obj["id"];
      ...
      if (options.create || options.delete) {
        ...
        // If we want to create a new class based on a subclass, go for this
        if (options.abstract) property = "rdfs:subClassOf ";
      ...
      for (property in obj) {
        ...
        if (attributes[property]) {
          ...
          if (attributes[property].model || attributes[property].collection) {
            var foreign = attributes[property];
...
//					external_prefix = shared.connections[external_connection].prefix.abr;
          ...
          /*
           * else, use brackets for Datatype Properties (and add language code if necessary.
           * e.G. <entry.12345> <has_title> "legalize it"@en
           */
          else {
            triples += property_string + ' "' + obj[property] + '" ' + end + '\n';
          }
        }
