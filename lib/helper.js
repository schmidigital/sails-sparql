...
console.log();
	 ...
	 *
	 * @param {Object ¡¡ Array}  values  values to convert
	 * @param {Object} options
	 * 					- {Array} custom_type Array of custom types which to add to the model
	 */
	model_to_turtle: function(connection, collection, values, options) {
		...
		// If we have a custom type (or custom types), create each type seperately
		if (options.custom_type) {
			console.log("jo")
			for (custom_collection in values) {
				return this.model_to_turtle(connection, custom_collection, values[custom_collection])
			}
		}
		else {
			for (value in values) {
