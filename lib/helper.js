...
var sparrow = module.exports = {
        ...
        function deep(branch, parent_condition, deep_collection, stack, offset) {
            ...
            for (var condition in branch) {
                ...
                //else if (deep_collection_attributes[condition]) {
					 ...
					 *
					 ********************************/
					else if (deep_collection_attributes[property_key].model || deep_collection_attributes[property_key].collection) {
						var uri = sparrow.ensure_prefix(branch[condition], conn.prefix.abr);
						where_clause += "?" + stack + " "
     ...
     */
    model_to_turtle: function (connection, collection, values, options, cb) {
		...
		async.forEachOf(values, function (keyvalue, value, callback) {
			...
			async.forEachOfSeries(obj, function (keyproperty, property, callback2) {
				...
				if (attributes[property] || (columnName = _.findKey(attributes, { columnName: property }))) {
					...
					async.waterfall([
						...
						function (callback3) {
							if ((attributes[property_key].model || attributes[property_key].collection)) {
								...
								var external_type = attributes[property_key]['model'] || attributes[property_key]['collection'];
								...
								setTimeout(function () {
									sparrow._ask(conn, ask_query, {}, function(err, res) {
										...
										if (res.search("false") < 0) {
											triples += end + "\n" + property_string + ' ' + external_prefix + obj[property];
											callback3(null, true)
										}
										else {
						...
						function(value_added, callback3) {
							...
							if (!(attributes[property_key].model || attributes[property_key].collection)) {
								...
								if (value == null) {
									...
									triples += end + '\n' + property_string + ' ' + types.toSparqlInsert[type](value) + ' ';
									callback3(null, true)
								}
					...
					function(err, value_added) {
						console.log("Finished with: ", property_key)
						console.log("Value Added?", value_added)
