							...
							if ((attributes[property_key].model || attributes[property_key].collection) || attributes[property_key].foreignKey) {
								...
								if (!options.delete) {
									...
									/*
									 ...
									 { columnName: 'drivercustom_taxis',
									 type: 'integer',
									 foreignKey: true,
									 references: 'drivercustom',
									 on: 'number',
									 onKey: 'number',
									 via: 'taxicustom_drivers',
									 groupKey: 'drivercustom' }
									...
									// OK we need to get the ID of the model with custom PK. So we can create the association
										...
										ask_query += external_prefix + foreign_key + " " + types.toSparqlQuery[attributes[property_key].type](obj[property]) + '}\n'
									}
